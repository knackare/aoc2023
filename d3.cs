...
internal static class d3
{
    ...
    {
        var rows = halp.lines(input);

        var sympos = new List<(int x, int y, char symbol)>();

        int colslen = rows[0].Length;
        ...
        //Console.WriteLine(sympos.Count);

        var adjacent = new List<int>();

        foreach (var (x, y, symbol) in sympos)
        {
            addadjacent(x, y, symbol);
        }

        return adjacent.Sum();

        void addadjacent(int x, int y, char symbol)
        {
            ...
            for (int py = y - 1; py <= y + 1; py++)
            {
                ...
                {
                    if (int.TryParse(rows[py][px].ToString(), out _))
                    {
                        var adj = getnum(px, py);
            ...
            for (int py = y - 1; py <= y + 1; py++)
            {
                for (int px = x + 1; px <= x + 1; px++)
                {
                    if (int.TryParse(rows[py][px].ToString(), out _))
                    {
                        var adj = getnum(px, py);
                        //Console.WriteLine(adj);
                        if (adjs.Contains(adj)) break; // overlap check hack
            ...
            // part 2
            if (symbol == '*' && adjs.Count == 2)
            {
                adjacent.Add(adjs[0] * adjs[1]);
            }
        ...
        int getnum(int px, int py)
        {
            ...
            {
                var test = --col;
                if (!int.TryParse(row[test].ToString(), out _))
                    break;
            ...
            while (nums.Count != 0) { sb.Append(nums.Pop()); }
            ...
            int adj = int.Parse(sb.ToString());
            return adj;
        }

    }

    private static readonly string test = @"467..114..
...*......
..35..633.
......#...
617*......
...
.664.598.."; // 4361
    ...
    private static readonly string test2 = @"467..114..
...*......
..35..633.
......#...
617*......
...
.........."; // 3606
    ...
    private static readonly string input = @"...................15....904...........850.................329...................13....................................871....816....697....
