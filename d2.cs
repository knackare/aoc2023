    ...
    internal class d2
    {
        internal static long solve()
        {
            var games = halp.lines(input).Select(line => {
                ...
                foreach (string s in foo[1].Split(";"))
                {
                    var red = 0;
                    var green = 0;
                    var blue = 0;
                    foreach(string cubes in s.Split(","))
                    {
                        var bar = cubes.Split(" ", StringSplitOptions.RemoveEmptyEntries);
                        switch (bar[1])
                        {
                            case "red":
                                red = int.Parse(bar[0]); break;
                            case "green":
                                green = int.Parse(bar[0]); break;
                            case "blue":
                                blue = int.Parse(bar[0]);break;
                            default:
                                throw new Exception(bar.Length.ToString());
                        }
                    }
            ...
            {
                Console.WriteLine($"{game.gameNo}: {string.Join('|', game.sets)}");
            }

            var max = (red: 12, green: 13, blue: 14);

            var possible = games.Where(game => {
                foreach (var (red, green, blue) in game.sets)
                {
                    if (red > max.red) { return false; }
                    if (green > max.green) { return false; }
                    if (blue > max.blue) { return false; }
                }

                return true;
            });

            //return possible.Select(g => g.gameNo).Sum(); // part 1

            var minimum = games.Select(game => {
                var reds = game.sets.Select(x => x.red).Max();
                var greens = game.sets.Select(x => x.green).Max();
                var blues = game.sets.Select(x => x.blue).Max();
...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";
        ...
        private static readonly string input = @"Game 1: 3 blue, 7 green, 10 red; 4 green, 4 red; 1 green, 7 blue, 5 red; 8 blue, 10 red; 7 blue, 19 red, 1 green
